...
extension DetailMenuVC : ListOptionDelagate {
  func checkdidTap(_ cell: UITableViewCell) {
    ...
    data!.optionGroup[indexPath.section - 2]!.option[index].selectPos = index == (indexPath.row - 1)
      ...
      let groupItem = OrderData.OptionGroup(id: menuItem.id, name: menuItem.name, menuId: menuItem.optionGroupId, mandatory: menuItem.selectPos, price: menuItem.price)
      
      if selectPos {
        self.orderlist?.optiongroup.append(groupItem)
        print("orderlist.optiongroup: \(self.orderlist?.optiongroup)")
        reCalculatePriceAndDisplay()
        tableView.reloadData()
      } else {
        self.orderlist?.optiongroup.removeAll()
      }
    }
    tableView.reloadData()
